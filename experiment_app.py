...
try:
    ...
    if run_ai and not is_instrument:
        ...
        prompt = f"""당신은 대학 실험 보고서를 작성하는 조교입니다. 학생이 업로드한 실험 데이터를 기반으로 보고서 결과 및 고찰에 사용할 문장을 작성합니다. 출력은 반드시 다음 세 문장으로만 작성합니다.

1. X 증가에 따른 Y의 변화 추세
2. 회귀분석(R², 이상치 포함)을 이용한 데이터 신뢰성
3. 이 데이터로부터 얻을 수 있는 결론 및 활용 가능성

각 문장은 60자 이하로 작성하십시오.

추측은 하지 마십시오.

데이터에 없는 내용은 절대 작성하지 마십시오.업로드된 데이터의 X축 이름, Y축 이름, 회귀분석 결과(R², 기울기), 이상치 여부를 기반으로 분석합니다. 또한, 절대 추측하지 않습니다. 주어진 정보만 이용하여 답변합니다.
출력은 반드시 아래 형식을 따릅니다.
...
4. 한계점 및 추가 분석 제안
 ...
 "trend":"농도가 증가함에 따라 흡광도가 증가하는 선형 경향을 보인다.",
 "validity":"R²가 0.984로 매우 높으며 선형 회귀 모델이 데이터를 잘 설명한다. 이상치 1개가 존재하므로 해당 지점은 추가 검토가 필요하다.",
 "interpretation":"본 데이터는 농도와 흡광도의 관계를 정량적으로 분석하는 데 활용 가능하며 Beer-Lambert 법칙을 검증하는 자료로 사용할 수 있다."
